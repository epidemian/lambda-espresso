...
// Generated by CoffeeScript 1.7.1
(function() {
  var $error, $errorContainer, $examplesMenu, $input, $output, $outputContainer, arrowHtml, arrowHtmlByType, arrowLabel, arrowSymbol, example, examples, getOptions, hash, i, lambda, preserveScrollPosition, renderCollapsedReduction, renderExpandedReduction, renderReductions, renderStepOptions, run, synonymsHtml, termHtml, timed, updateInputFromHash, _i, _len;
  ...
  renderCollapsedReduction = function(_arg) {
    var collapsed, final, finalHtml, finalSynonyms, initial, totalSteps;
    initial = _arg.initial, final = _arg.final, finalSynonyms = _arg.finalSynonyms, totalSteps = _arg.totalSteps;
    finalHtml = totalSteps > 0 ? "" + (arrowHtml('→', "(" + totalSteps + ")")) + " " + (termHtml(final)) : '';
    collapsed = "" + (termHtml(initial)) + " " + finalHtml + " " + (synonymsHtml(finalSynonyms));
    return "<div class=reduction><div class=collapsed>" + collapsed + "</div></div>";
  };

  renderExpandedReduction = function(_arg) {
    var afterHtml, arrow, beforeHtml, finalSynonyms, i, initial, renderStep, step, steps, synonyms, totalSteps;
    ...
    steps = (function() {
      ...
      for (i = _i = 0; 0 <= totalSteps ? _i < totalSteps : _i > totalSteps; i = 0 <= totalSteps ? ++_i : --_i) {
        ...
        synonyms = i === totalSteps - 1 ? synonymsHtml(finalSynonyms) : '';
        _results.push("<span class=step>" + beforeHtml + "<br>" + arrow + " " + afterHtml + " " + synonyms + "</span>");
      }
      return _results;
    })();
