...
require['./lambda'] = new function() {
  ...
  // Generated by CoffeeScript 1.3.3
(function() {
  var Abstraction, Application, Macro, Variable, applyStep, isFree, leftApplicationStr, parse, parseTerm, reduceStep, reduceTerm, renameVar, rightApplicationStr, substitute, termStr, type, varRenameCollides;
  ...
  termStr = function(t) {
    switch (type(t)) {
      ...
      case Macro:
        return t.name;
      case Abstraction:
        return "λ" + t.varName + "." + (termStr(t.body));
      case Application:
        return "" + (leftApplicationStr(t.left)) + " " + (rightApplicationStr(t.right));
    }
  };

  leftApplicationStr = function(t) {
    if ((type(t)) === Abstraction) {
      return "(" + (termStr(t)) + ")";
    } else {
      return termStr(t);
    }
  ...
  rightApplicationStr = function(t) {
    if ((type(t)) === Application) {
      return "(" + (termStr(t)) + ")";
    } else {
      return termStr(t);
    }
  ...
  reduceStep = function(t) {
    var applied, reducedBody, reducedLeft, reducedRight;
    switch (type(t)) {
      case Variable:
        return null;
      case Abstraction:
        reducedBody = reduceStep(t.body);
        return reducedBody && new Abstraction(t.varName, reducedBody);
      case Application:
        ...
        if (reducedLeft) {
          return new Application(reducedLeft, t.right);
        }
        reducedRight = reduceStep(t.right);
        return reducedRight && new Application(t.left, reducedRight);
      case Macro:
        return (reduceStep(t.term)) && t.term;
    }
  };
  ...
  applyStep = function(t, s) {
    switch (type(t)) {
      ...
      case Application:
        return null;
      case Abstraction:
        return substitute(t.body, t.varName, s);
      case Macro:
        return (applyStep(t.term, s)) && new Application(t.term, s);
    }
  };
  ...
  substitute = function(t, x, s) {
    switch (type(t)) {
      ...
      case Abstraction:
        ...
        if ((isFree(t.varName, s)) && (isFree(x, t.body))) {
          return substitute(renameVar(t, s), x, s);
        } else {
          return new Abstraction(t.varName, substitute(t.body, x, s));
        }
        break;
      case Application:
        return new Application(substitute(t.left, x, s), substitute(t.right, x, s));
      case Macro:
        if (isFree(x, t.term)) {
  ...
  renameVar = function(abstraction, t) {
    var base, body, isValid, m, n, newName, varName;
    varName = abstraction.varName, body = abstraction.body;
    base = varName.replace(/\d+$/, '');
    n = (m = varName.match(/\d+$/)) ? parseInt(m[0]) : 0;
    while (!(newName && isValid)) {
      newName = base + ++n;
      isValid = !(isFree(newName, t)) && !(isFree(newName, body)) && !(varRenameCollides(body, varName, newName));
    }
    return new Abstraction(newName, substitute(body, varName, new Variable(newName)));
  };
  ...
  isFree = function(x, t) {
    switch (type(t)) {
      case Variable:
        return t.name === x;
      case Abstraction:
        return t.varName !== x && isFree(x, t.body);
      case Application:
        return (isFree(x, t.left)) || (isFree(x, t.right));
      case Macro:
        return isFree(x, t.term);
    }
  };
  ...
  varRenameCollides = function(t, oldName, newName) {
    ...
    switch (type(t)) {
      case Variable:
        return false;
      case Abstraction:
        collisionHere = (isFree(oldName, t)) && t.varName === newName;
...
require['./index'] = new function() {
  ...
  // Generated by CoffeeScript 1.3.3
(function() {
  var $error, $errorContainer, $examplesMenu, $input, $output, $outputContainer, examples, lambda, options, preserveScrollPosition, run, updateInputFromHash, updateOutputExpansions;
  ...
  run = function() {
    var collapsed, i, program, reductions, result, step, steps, _i, _j, _len, _len1;
    program = $input.val();
    try {
      ...
      for (_i = 0, _len = reductions.length; _i < _len; _i++) {
        ...
        collapsed = steps.length === 1 ? "<b>" + steps[0] + "</b>" : "" + steps[0] + " → <b>" + steps[steps.length - 1] + "</b>";
