...
// Generated by CoffeeScript 1.6.3
(function() {
  var $error, $errorContainer, $examplesMenu, $input, $output, $outputContainer, arrowHtml, arrowHtmlByType, arrowLabel, arrowSymbol, example, examples, getOptions, hash, i, lambda, preserveScrollPosition, renderExpandedReduction, renderReductions, renderStepOptions, run, termHtml, timed, updateInputFromHash, _i, _len;
  ...
  renderReductions = timed('render html', function(reductions) {
    var collapsed, final, initial, result, totalSteps, _i, _len, _ref;
    result = '';
    for (_i = 0, _len = reductions.length; _i < _len; _i++) {
      _ref = reductions[_i], initial = _ref.initial, final = _ref.final, totalSteps = _ref.totalSteps;
  ...
  renderExpandedReduction = function(_arg) {
    var after, before, expanded, i, initial, renderStep, totalSteps, type, _i, _ref;
    totalSteps = _arg.totalSteps, initial = _arg.initial, renderStep = _arg.renderStep;
    ...
    if (totalSteps === 0) {
      ...
      for (i = _i = 0; 0 <= totalSteps ? _i < totalSteps : _i > totalSteps; i = 0 <= totalSteps ? ++_i : --_i) {
        ...
        expanded += '<span class="step">' + (termHtml(before, 'before')) + '<br>' + (arrowHtmlByType(type)) + (termHtml(after, 'after')) + '</span>';
      }
    }
    expanded += '</div>';
    return expanded;
...
// Generated by CoffeeScript 1.6.3
(function() {
  var Abstraction, AlphaConversionStep, Application, BetaReductionStep, Macro, MacroExpansionStep, Step, Variable, apply, applyStep, compose, composeAbs, composeAppL, composeAppR, defaultOptions, expandStep, extend, find, freeIn, highlight, highlightAbstractionVar, identity, markStep, parse, parseTerm, reduceCallByName, reduceNormal, reduceStep, reduceTerm, renameStep, renameVar, repeatStr, replace, substitute, substitute2, termStr, termTreeStr, timed, varRenameCollides, wrapStep, _ref,
    __slice = [].slice;
  ...
  AlphaConversionStep = function(abst, renamedAbst) {
    var after, before;
    ...
    return Step('alpha', before, after, renamedAbst);
  };

  MacroExpansionStep = function(t) {
    var after, before;
    before = highlight(t, highlightSubstituted);
    after = highlight(t.term, highlightSubstitutionTerm);
    return Step('macro', before, after, t.term);
  };
  ...
  reduceStep = function(t) {
    ...
    switch (t.type) {
      ...
      case Macro:
        return (reduceStep(t.term)) && MacroExpansionStep(t);
    }
  };
    ...
    switch (t.type) {
      ...
      case Macro:
        if (applyStep(t.term, s)) {
          return wrapStep(MacroExpansionStep(t), function(macro) {
    ...
    switch (t.type) {
      ...
      case Abstraction:
        ...
        if ((freeIn(t.varName, s)) && (freeIn(x, t.body))) {
          ...
          return AlphaConversionStep(t, Abstraction(newVarName, newBody));
        } else {
    ...
    switch (t.type) {
      ...
      case Abstraction:
        ...
        if ((freeIn(t.varName, s)) && (freeIn(x, t.body))) {
          ...
          return Abstraction(newVarName, substitute2(renamedBody, x, s));
        } else {
          return Abstraction(t.varName, substitute2(t.body, x, s, cb && composeAbs(cb, t.varName)));
        }
        break;
      case Application:
        l = substitute2(t.left, x, s, cb && composeAppR(cb, t.right));
        r = substitute2(t.right, x, s, cb && composeAppL(cb, t.left));
        return Application(l, r);
      case Macro:
        if (freeIn(x, t.term)) {
          throw Error(("Logical error: " + x + " is free in " + t.name + ".") + "Macros cannot have free variables");
  ...
  replace = function(t, replaced, replacement) {
    var body, l, r;
    if (t === replaced) {
      return replacement;
    }
  ...
  expandStep = function(t, options) {
    ...
    console.log('expandStep', options);
  ...
  reduceTerm = timed('reduce', function(term, options) {
    var e, enough, final, initial, maxSteps, renderStep, steps, terminates, totalSteps;
    ...
    final = termStr(steps[steps.length - 1] || term);
    totalSteps = steps.length;
    renderStep = function(i, options) {
      console.log('renderStep', options);
  ...
  exports.reduceProgram = function(expr, options) {
    ...
    for (_i = 0, _len = terms.length; _i < _len; _i++) {
      term = terms[_i];
      _results.push(reduceTerm(term, options));
    }
    return _results;
  };
